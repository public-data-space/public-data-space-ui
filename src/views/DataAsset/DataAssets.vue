    ...
    <v-card-text>
      <v-container style="display:flex;align-items:center;padding:initial;">
        ...
        <v-select
          ...
          clearable></v-select>
        <v-switch :disabled="dataassets.length==0" label="Published" color="accent" class="switch-label" v-model="filterPublished"></v-switch>
        <v-switch :disabled="dataassets.length==0" label="Unpublished" color="grey" class="switch-label" v-model="filterUnpublished"></v-switch>
        <v-btn :disabled="dataassets.length==0" v-if="checkForUnpublished" @click="publishAll" color="accent" class="publishAll mr-1">Alle Veröffentlichen</v-btn>
        <v-btn :disabled="dataassets.length==0" v-if="checkForPublished" @click="unpublishAll" color="grey" dark class="unPublishAll">Alle Zurückhalten</v-btn>
      </v-container>
      <v-data-table 
        :headers="headers"
        :items="filteredItems"
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: 'Datasource Type', value: 'datasourceType' },
      ...
      datasourceType: '',
      datasourceName: '',
      snackbarTextPublishSuccess: GLUE_CONFIG.snackbarTexts.dataAssets.publish.success,
      snackbarTextPublishError: GLUE_CONFIG.snackbarTexts.dataAssets.publish.error,
      snackbarTextUnpublishSuccess: GLUE_CONFIG.snackbarTexts.dataAssets.unpublish.success,
      snackbarTextUnpublishError: GLUE_CONFIG.snackbarTexts.dataAssets.unpublish.error,
      snackbarTextDeleteSuccess: GLUE_CONFIG.snackbarTexts.dataAssets.delete.success,
      snackbarTextDeleteError: GLUE_CONFIG.snackbarTexts.dataAssets.delete.error,
    };
  },
  beforeMount() {
    this.loadData()
  ...
  computed: {
    ...
    checkForPublished() {
      return this.dataassets.filter(da => da.status === 'PUBLISHED').length > 0;
    },
    checkForUnpublished() {
      return this.dataassets.filter(da => da.status === 'APPROVED').length > 0;
    },
    mungedDatasets() {
    ...
    filteredItems() {
      ...
      let filteredDataassetsByStatus = this.dataassets
        ...
        .filter(da => !this.filterUnpublished ? da.status === 'PUBLISHED' : da);
        // .filter(da => da)
        // .filter(da => da)
        // .filter(da => da);
  ...
  methods: {
    ...
    publishAll() {
      ...
      .then(() => {
        this.$store.dispatch('updateStatus', { status: 'success', text: this.snackbarTextPublishSuccess });
        this.loadData();
      })
      .catch(() => {
        this.$store.dispatch('updateStatus', { status: 'error', text: this.snackbarTextPublishError });
      });
    },
    unpublishAll() {
      this.$axios({
      ...
      .then(() => {
        this.$store.dispatch('updateStatus', { status: 'success', text: this.snackbarTextUnpublishSuccess });
        this.loadData();
      })
      .catch(() => {
        this.$store.dispatch('updateStatus', { status: 'error', text: this.snackbarTextUnpublishError });
      });
    },
    publishAction(id) {
      this.$axios({
    ...
    deleteAction(id) {
      ...
      .then(() => {
        this.$store.dispatch('updateStatus', { status: 'success', text: this.snackbarTextDeleteSuccess });
        this.loadData();
      })
      .catch(() => {
        this.$store.dispatch('updateStatus', { status: 'error', text: this.snackbarTextDeleteError });
      });
    },
    rowClick(item) {
